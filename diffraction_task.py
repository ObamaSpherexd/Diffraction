...
def make_diffraction_grating_mask(N, size, period, duty_cycle):
    """Маска дифракционной решетки (прозрачные щели с непрозрачными полосами)."""
    x = np.linspace(-size / 2, size / 2, N)
    y = np.linspace(-size / 2, size / 2, N)
    X, Y = np.meshgrid(x, y)
...
def main():
    ...
    with col1:
        ...
        preset_names = ["-- Выберите пресет --"] + [p.name for p in PRESETS]
        ...
        mode = st.radio("Режим", ["auto", "fresnel", "fraunhofer"], horizontal=True)

        params_key = (
            wavelength_nm, a, b, aperture_size_mm, screen_size_mm,
            grid_points, ap_type, tuple(sorted(params.items()))
        )
        ...
        if True:
            try:
                ...
                intensity, x, y, mode_used, N_F = compute_diffraction(
                    aperture, wavelength, a, b, aperture_size, screen_size, N, mode=mode)
